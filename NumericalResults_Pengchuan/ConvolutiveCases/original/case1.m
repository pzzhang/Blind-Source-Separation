...
if hKnown_choice>=1
    ...
    mixed_x=loadmix(premix_choice);      q=50;100;
end
...
[envelope_X,vX,sX]=RecoverSource(mixed_x,X,q,'from full LM',show_s_choice);
if hKnown_choice>=1
	[envelope_Xex,vXex,sXex]=RecoverSource(mixed_x,Xex,q,'from exact',show_s_choice);
...
if sparse_choice>=1
	[envelope_Xs,vXs,sXs]=RecoverSource(mixed_x,Xs,q,'from sparse LM with estimated init position',show_s_choice);
end

if hKnown_choice>=1
    myfigure; 
    for i=1:2
        subplot(2,1,i); plot(envelope_Xex(i,:),'b'); hold on; plot(envelope_X(i,:),'r:'); hold on;
        if sparse_choice>=1
            plot(envelope_Xs(i,:),'g-.');  legend('Ex','FLM ','SLMs');  
        else
            legend('Ex','FLM'); 
        end
        %axis([0,160*256,0,1])
...
temp=barrho(casenum,20,vX(1,:),vX(2,:));   disp(['rhobar of v from full LM = ',num2str(temp,'%5.5f')]);
temp=barrho(casenum,20,sX(1,:),sX(2,:));   disp(['rhobar of s from full LM = ',num2str(temp,'%5.5f')]);



% case 1:
%subplot(2,1,2);  title('v: background music');
%subplot(2,1,1);  title('v: counting digits (1 to 10)');
%axis([0,110000,-0.015,0.015])

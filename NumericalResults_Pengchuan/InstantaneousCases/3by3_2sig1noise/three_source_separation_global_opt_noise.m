...
theta3 = pi/2;
phi3 = pi/12;
x0 = [1/6 1/5 1/4 1/3 1/6+1e-3 1/5+1e-3]*pi;
...
% initial guess
% xperturb = 4;
% x0 = initialGuess( ms,cs );
% X0 = x0 - xperturb*pi/4*(2*rand(1,6)-1);
% for i = 1 : 3
%     X0(2*i-1) = 2*pi*(X0(2*i-1)/(2*pi)-floor(X0(2*i-1)/(2*pi)))-pi;
%     X0(2*i) = pi*(X0(2*i)/(pi)-floor(X0(2*i)/(pi)))-pi/2;
% end
% X0=[0.506263252421015,-0.954443934495913,-1.75904983790546,-0.217958228913639,-0.833061539627684,-0.942258007640129];
xseed = 2*rand(1,6)-1;
X0 = pi/2*[2,1,2,1,2,1].*xseed;
X0=[0.506263252421015,-0.954443934495913,-1.75904983790546,-0.217958228913639,-0.833061539627684,-0.942258007640129];
fvalopt = simple_objective(x0, ms);
fval0 = simple_objective(X0, ms);

% [x,fval,exitFlag,output] = simulannealbnd(ObjectiveFunction,X0)
% perturb = 1;
% lb = X0 - perturb*pi/4*ones(1,6);
% ub = X0 + perturb*pi/4*ones(1,6);
lb = -pi/2*[2,1,2,1,2,1];
ub = pi/2*[2,1,2,1,2,1]; 
